...
const StoreMap = ({isActiveStores,fileterStoreData,handleStoreSave,iconsStoreSave}) => {
  return (
    <View>
      {isActiveStores && (
        <ScrollView showsVerticalScrollIndicator={false}>
          {fileterStoreData.map((item, index) => {
            return (
              <View style={styles.hotelCardView} key={index}>
                ...
                <Pressable
                  style={styles.Icon}
                  onPress={() => {
                    handleStoreSave(item, index);
                  }}
                >
                  {iconsStoreSave[index] ? (
